    ...
    sort: function(th, direction) {
      var start = new Date(),
        ...
        naturalSort = function naturalSort(a, b) {
          var chunkRegExp = /(^-?[0-9]+(\.?[0-9]*)[df]?e?[0-9]?$|^0x[0-9a-f]+$|[0-9]+)/gi,
            stripRegExp = /(^[ ]*|[ ]*$)/g,
            ...
            xDate =
              parseInt(x.match(numericRegExp), 10) ||
              (xChunked.length != 1 && x.match(dateRegExp) && Date.parse(x)),
            yDate =
              parseInt(y.match(numericRegExp), 10) ||
              (xDate && y.match(dateRegExp) && Date.parse(y)) ||
              null,
            xHexValue,
            yHexValue,
            index;
          ...
          for (index = 0; index < chunkLength; index++) {
            ...
            xHexValue =
              (!(xChunked[index] || '').match(oRegExp) &&
                parseFloat(xChunked[index])) ||
              xChunked[index] ||
              0;
            yHexValue =
              (!(yChunked[index] || '').match(oRegExp) &&
                parseFloat(yChunked[index])) ||
              yChunked[index] ||
              0;
      ...
      if (this.index != index) {
        ...
        this.direction[index] =
          this.direction[index] === 'desc' ? 'asc' : 'desc';
      } else {
