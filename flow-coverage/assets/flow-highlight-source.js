$(function() {
  ...
  var cm = CodeMirror.fromTextArea(document.querySelector('textarea'), {
    ...
    gutters: ['Flow-lineuncovered', 'CodeMirror-linenumbers'],
  });

  // TODO: handle corrupted coverage data?
  var coverageData = JSON.parse(
    document.querySelector('#file-coverage-data').textContent,
  );
  ...
  scrollbarAnnotations.update(
    uncoveredLocs.map(function(loc) {
      return {
  ...
  $('.ui.dropdown.uncovered-locations').dropdown({
    action: 'hide',
  ...
  $('.ui.dropdown.syntax-highlighting').dropdown({
    onChange: function(value) {
      var mode = 'typescript';
